...
async function startServer() {
  ...
  addLog("PLATFORM_API", "info", "Initializing Digital Pathways 3.0 Platform Backend engine.");
  ...
  app.post("/api/opportunities/:id/questions", (req, res) => {
    try {
      ...
      // Notify coordinator of new student inquiry
      if (opp.poster_email) {
        const queryNotifier = {
          id: `notif-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
          email: opp.poster_email.toLowerCase(),
          message: `New question from <${user_email}> on your program "${opp.title}": "${text.length > 40 ? text.substring(0, 40) + '...' : text}"`,
          timestamp: new Date().toISOString(),
          read: false,
  ...
  app.post("/api/questions/:questionId/answer", (req, res) => {
    try {
      ...
      // Add notification for the question asker
      const notif = {
        id: `notif-${Date.now()}`,
        email: question.user_email,
        message: `Coordinator replied to your question in "${opp ? opp.title : 'Opportunity'}": "${question.text.length > 40 ? question.text.substring(0, 40) + '...' : question.text}" -> "${answer.trim()}"`,
        timestamp: new Date().toISOString(),
        read: false,
  ...
  const handlePostOpportunity = (req: express.Request, res: express.Response) => {
    try {
      const { title, description, date, category, poster_email, fields, seats, extra_info, cover_photo, difficulty, location_state, testimonials, required_age } = req.body;
      
      if (!title || !description || !date || !category || !poster_email || !fields) {
        res.status(400).json({ error: "Missing required opportunity fields. Please ensure basic parameters, contact, and form schema are set." });
        return;
      }
      ...
      if (opportunity) {
        ...
        if (wasWaitlisted) {
          ...
          if (waitlistedRegs.length > 0) {
            ...
            const email = (candidateToPromote.responses["Email Address"] || candidateToPromote.responses["Email"] || candidateToPromote.responses["email"] || "").toString().toLowerCase();
            if (email) {
              const notif = {
                id: `notif-${Date.now()}`,
                email,
  ...
  const handleKickRegistration = (req: express.Request, res: express.Response) => {
    try {
      ...
      if (dismissedUserEmail) {
        ...
        db.notifications = db.notifications || [];
        db.notifications.push(notif);
        addLog("SMTP_SERVER", "info", `SMTP dispatch: student dismissed notification sent to <${dismissedUserEmail}>.`, { notification: notif });
      }
  ...
  app.get("/api/notifications", (req, res) => {
    try {
      ...
      const db = loadDB();
      const list = (db.notifications || []).filter((n: any) => n.email.toLowerCase() === email.toLowerCase());
      res.json(list);
    } catch (e: any) {
  ...
  app.post("/api/notifications/clear", (req, res) => {
    try {
      ...
      const db = loadDB();
      db.notifications = (db.notifications || []).filter((n: any) => n.email.toLowerCase() !== email.toLowerCase());
      saveDB(db);
  ...
  app.post("/api/auth/register", (req, res) => {
    try {
      ...
      if (!name || !email || !password || !avatarUrl || !role || age === undefined) {
        res.status(400).json({ error: "Missing identity attributes. Name, email, password, age, avatar selection, and designation are required." });
        return;
      }

      const parsedAge = Number(age);
      if (isNaN(parsedAge) || parsedAge <= 0 || parsedAge > 120) {
        res.status(400).json({ error: "Invalid Age: Please provide a valid, positive age between 1 and 120." });
        return;
      }
