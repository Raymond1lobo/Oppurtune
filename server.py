...
async def api_signup(request: Request):
    ...
    client_user = {"name": name, "email": email, "avatarUrl": avatarUrl, "role": role}
...
def main():
    ...
    # Select GUI or Headless CLI automatically
    if not args.headless and HAS_GUI:
        try:
            print("* Starting visual administration dashboard launcher...")
            run_server_gui(port, args.debug)
            return
        except Exception as e:
            print(f"[GUI Init Failed]: {e}. Falling back to standard CLI terminal...")
